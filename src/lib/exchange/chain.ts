...
export type ChainListing = ChainWeapon & { id: string; seller: string; buyer: string; priceRaw: string; priceSilo: string; receiptMint: string; status: 'active' | 'sold' | 'cancelled'; name: string };
...
async function library(): Promise<Library> {
  ...
  if (!loaded) loaded = new Promise((resolve, reject) => {
    ...
    script.src = '/economy/silo-economy.js?v=20260918-economy-2';
...
export function formatSilo(raw: bigint): string { return `${raw / 1_000_000_000n}${raw % 1_000_000_000n ? '.' + (raw % 1_000_000_000n).toString().padStart(9, '0').replace(/0+$/, '') : ''}`; }
export function parseSilo(value: string): bigint {
  if (!/^\d+(?:\.\d{1,9})?$/.test(value)) throw new Error('Use a SILO amount with at most nine decimal places.');
  const [whole, fraction = ''] = value.split('.'); const raw = BigInt(whole) * 1_000_000_000n + BigInt(fraction.padEnd(9,'0'));
  if (raw < 20n || raw > 1_000_000_000_000_000_000n) throw new Error('Enter a valid positive price.'); return raw;
}
export function quoteSilo(value: string) {
  try { const price = parseSilo(value), burn = price / 20n, transferred = price - burn, fee = (transferred + 99n) / 100n;return { valid: true, price: formatSilo(price), burn: formatSilo(burn), fee: formatSilo(fee), seller: formatSilo(transferred-fee) }; }
  catch { return { valid: false, price: '0', burn: '0', fee: '0', seller: '0' }; }
}
...
export async function readListings(): Promise<ChainListing[]> {
  return (await (await client()).listMarket()).map(row=>{const item=weapon(row.weapon);return {...item,id:row.address,seller:row.seller,buyer:row.buyer,priceRaw:row.price.toString(),priceSilo:formatSilo(row.price),receiptMint:row.receiptMint,status:(['active','sold','cancelled'] as const)[row.status],name:`SF ${CLASS_LABEL[item.classId]}`};});
}
...
export async function listWeapon(owner: string, mint: string, price: string) { parseSilo(price);const c=await client();assertWallet(owner);return c.listWeapon(mint,price); }
